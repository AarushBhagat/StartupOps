...
export const LeaderDashboard = ({ 
  ...
  const stats = hasStartupInfo ? [
    // Dynamic based on the milestone tasks we have
    { label: 'Total Milestones', value: roadmap?.milestones?.length || '0', icon: Target, color: 'from-cyan-500 to-blue-600' },
    { label: 'Completed', value: roadmap?.milestones?.filter((m: any) => m.status === 'completed').length || '0', icon: CheckCircle2, color: 'from-green-500 to-emerald-600' },
    { label: 'Est. Completion', value: `${roadmap?.estimatedCompletion || '12'} wks`, icon: Clock, color: 'from-purple-500 to-pink-600' },
