...
${startupProfile?.problemStatement || 'We are addressing a critical challenge in our target market.'}

**Our Solution**

${startupProfile?.solution || 'Our innovative platform solves this problem efficiently.'}

**Market Opportunity**

Our target audience is ${startupProfile?.targetAudience || 'a growing market segment'}. We are uniquely positioned to serve them.

**Traction & Progress**

• ${metrics.users} active users growing at ${metrics.growth}% month-over-month
...
Our founding team brings deep expertise to execute this vision.
...
Seeking seed funding to expand our team, scale marketing, and enhance our product capabilities.`;
  ...
  if (user?.roadmap?.milestones) {
    const totalMilestones = user.roadmap.milestones.length;
    const completedMilestones = user.roadmap.milestones.filter((m: any) => m.status === 'completed').length;
    // Base 5 points per task, +10 points if completed, cap at 100
    pitchScore = Math.min(100, Math.floor((totalMilestones * 5) + (completedMilestones * 10)));
  }

  const scoreItems = [
    { label: 'Problem statement defined', status: startupProfile?.problemStatement ? 'complete' : 'missing', icon: startupProfile?.problemStatement ? CheckCircle : XCircle },
    { label: 'Solution articulated', status: startupProfile?.solution ? 'complete' : 'missing', icon: startupProfile?.solution ? CheckCircle : XCircle },
    { label: 'Team profiles complete', status: 'warning', icon: AlertCircle },
  ...
  const dueDiligenceDocs = [
    ...
    { label: 'Pitch deck', status: 'missing' },
      ...
      <div className="grid lg:grid-cols-2 gap-8 mb-8">
        ...
        >
          ...
          <div className="space-y-6 mb-8">
            <div>
              <div className="text-2xl font-bold text-white mb-1">{startupProfile?.startupName || 'Your Startup'}</div>
              <div className="text-gray-400">{startupProfile?.elevatorPitch || 'Elevator pitch goes here'}</div>
            </div>

            <div className="flex items-center gap-4 text-sm mt-4 mb-4">
              <span className="px-3 py-1 rounded-full bg-cyan-500/20 text-cyan-400">Pre-Seed</span>
              <span className="px-3 py-1 rounded-full bg-purple-500/20 text-purple-400">{startupProfile?.industry || 'Technology'}</span>
            </div>
            ...
            <div className="pt-4 border-t border-white/10">
              ...
              <div className="space-y-2 text-sm text-gray-300">
                <div className="flex justify-between">
                  ...
                  <span className="text-cyan-400">+{metrics.growth}% MoM</span>
                </div>
                {metrics.users === '0' && <div className="text-gray-500 italic text-xs">Awaiting traction data...</div>}
              </div>
            </div>
      ...
      <AnimatePresence>
        {showPitchModal && (
          <motion.div
            ...
            className="relative z-50 flex items-center justify-center p-4 mt-8"
          >
            <motion.div
