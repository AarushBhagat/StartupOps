...
export const StartupProfilePage = () => {
  ...
  const [profile, setProfile] = useState({
    ...
    tagline: startupProfile?.elevatorPitch || '',
    industry: startupProfile?.industry || '',
    stage: startupProfile?.stage || '',
    founded: '',
    website: '',
    problem: startupProfile?.problemStatement || '',
  ...
  const handleRemoveIdea = async () => {
    ...
    if (window.confirm('Are you sure you want to delete your idea and start over?')) {
      const userRef = doc(db, 'users', user.uid);
      await updateDoc(userRef, {
        startupProfile: deleteField(),
        roadmap: deleteField()
      });
        ...
        <div className="space-y-6">
          ...
          >
            ...
            <div className="space-y-4">
              ...
              {team.length === 0 && (
                <div className="text-gray-500 py-4 text-center">No team members added yet.</div>
              )}
            </div>
          </motion.div>
          ...
          >
            ...
            <div className="space-y-4">
              <div className="flex flex-col md:flex-row md:items-center justify-between p-4 rounded-xl bg-white/5 border border-white/10 gap-4">
                <div>
                  <div className="text-white font-medium mb-1">Remove My Idea</div>
                  <div className="text-sm text-gray-400">Clear all startup data and return to the idea generation step. Your account remains active.</div>
                </div>
                <button
                  onClick={handleRemoveIdea}
                  className="px-4 py-2 rounded-lg bg-red-500/20 text-red-400 font-medium hover:bg-red-500/30 transition-colors whitespace-nowrap"
                >
                  Start Fresh
                </button>
              </div>
              ...
              <div className="flex flex-col md:flex-row md:items-center justify-between p-4 rounded-xl bg-white/5 border border-white/10 gap-4">
                <div>
                  ...
                  <div className="text-sm text-gray-400">Permanently delete your account and all associated data. This cannot be undone.</div>
                </div>
                <button
                  onClick={handleDeleteAccount}
                  className="px-4 py-2 rounded-lg bg-red-500 text-white font-medium hover:bg-red-600 transition-colors whitespace-nowrap"
                >
                  Delete Account
                </button>
              </div>
