...
export default function App() {
  const [currentPage, setCurrentPageRaw] = React.useState(() => {
  ...
  React.useEffect(() => {
    if (user) {
      ...
      setHasStartupInfo(false);
      setStartupInfo(null);
      if (currentPage !== 'home' && currentPage !== 'plans' && currentPage !== 'login' && currentPage !== 'signup' && currentPage !== 'payment') {
        setCurrentPage('home');
      }
    }
  ...
  const handleSignupComplete = (plan?: string) => {
    // Signup completion is now handled by the useEffect auth listener which checks startupProfile
    // User will be routed to onboarding by the effect.
  };
  ...
  const handleOnboardingComplete = async (hasIdea: boolean, ideaData?: StartupInfo) => {
    if (hasIdea && ideaData) {
      setHasStartupInfo(true);
  ...
  const getUserName = () => {
    if (userRole === 'leader') return user?.displayName || 'Team Leader';
    if (userRole === 'team') return user?.displayName || 'Team Member';
